...
class GetMoviesByCategoryUsecase {
  ...
  Future<Result<MoviesByCategory>> execute() async {
    ...
    if (results case [
      ...
      final favoriteId = favorites.map((f) => f.id).toList();

      return Success(
        MoviesByCategory(
          popular: popular.markAsFavorite(favoriteId),
          topRated: topRated.markAsFavorite(favoriteId),
          nowPlaying: nowPlaying.markAsFavorite(favoriteId),
          upComing: upComing.markAsFavorite(favoriteId),
        ),
      );
    }

    return Failure(Exception('Error on get movies by category'));
  }
}
