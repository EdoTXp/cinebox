...
class GetMoviesByNameUsecase {
  ...
  Future<Result<List<Movie>>> execute({required String name}) async {
    ...
    if (results case [
      ...
      Success<List<Movie>>(value: final movieBySearch),
    ]) {
      final favoriteId = favorites.map((f) => f.id).toList();
      return Success(movieBySearch.markAsFavorite(favoriteId));
    }

    return Failure(Exception('Error on get movies by name'));
  }
}
